...
if user_menu == 'Overall Analysis':
    ...
    fig = px.line(region_over_time, x='Editions', y='region')
    ...
    x = helper.most_successful(df, selected_sport).head(15)
    st.table(x)
...
if user_menu == 'Country-wise Analysis':
    ...
    country_df = helper.year_wise_analysis(df, selected_country)
    ...
    if pt.empty:
        st.warning(selected_country + ' has no medal data available')
    else:
        fig, ax = plt.subplots(figsize=(20,20))
    ...
    top10_df = helper.top_10_athletes_by_country(df, selected_country)
...
if user_menu == 'Athelete-wise Analysis':
    ...
    selected_sport = st.selectbox('Select a Sport', sport_list)
    temp = helper.weight_v_height(df, selected_sport)
    fig, ax = plt.subplots()
    ax = sns.scatterplot(temp,x= 'Weight', y='Height',hue='Medal', style='Sex', s=60)
    st.pyplot(fig)
